...
class DepthFineTuner:
    ...
    def save_depth(self, dir: str = None, frames=None):
        ...
        for data in data_loader:
            ...
            final_depth_dir = pjoin(dir, "exact_depth")
            os.makedirs(final_depth_dir, exist_ok=True)
            final_depth_format = pjoin(final_depth_dir, "frame_{:04d}.dpt")
            final_depth = final_depth_format.format(frame_id+1)

            self.depth_write(final_depth,depth)
            depth.squeeze()
            inv_depth = 1.0 / depth
    ...
    def depth_write(self, filename, depth): #Copied from sintel_io.py from http://sintel.is.tue.mpg.de/depth
        """ Write depth to file. """
    ...
    def fine_tune(self, writer=None):
        ...
        # Training loop.
        total_iters = 0
        for epoch in range(self.params.num_epochs):  #TODO
...
############################Integrate the framework##############################################################
                ...
                Data_dir_mask = './data/human_depth/bandage_2_mask'
                Data_dir_depth = './data/human_depth/bandage_2_depth'
                frame=[0,0,0,0]d
                frame[0]=pairs[0][0]
                frame[1]=pairs[0][1]
                frame[2]=pairs[1][0]
                frame[3]=pairs[1][1]
                depth_frame=[]
                depth_frame.append(depth[0][0])
                depth_frame.append(depth[0][1])
                depth_frame.append(depth[1][0])
                depth_frame.append(depth[1][1])
                for frame_num in range(4):
                    path_mask = Data_dir_mask + '/' + 'frame_' +'%04d' %frame[frame_num] + '_mask.png'
                    path_depth = Data_dir_depth + '/' + 'frame_' +'%04d' %frame[frame_num] +'_depth_resize.txt'
                    if os.path.isfile(path_mask) and os.path.isfile(path_depth):
                       mask=cv2.imread(path_mask,-1)
                       print('mask shape is:')
                       print(np.shape(mask))
                       mask = cv2.resize(mask, (384, 160), interpolation=cv2.INTER_AREA)
                       depth_human=np.loadtxt(path_depth)
                       depth_human = cv2.resize(depth_human, (384,160),interpolation=cv2.INTER_AREA)
                       print('depth shape is:')
                       print(np.shape(depth_frame[frame_num].cpu().detach().numpy()))
                       L1_criterion=torch.nn.L1Loss()
                       error=L1_criterion(torch.from_numpy(mask*depth_frame[frame_num].cpu().detach().numpy()),torch.from_numpy(mask*np.mean(mask*depth_frame[frame_num].cpu().detach().numpy())/np.mean(mask*depth_human)*depth_human))
                       loss = loss + error
